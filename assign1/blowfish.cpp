...
{
  IvSet = false;
  if (hexKey.length () % 2 != 0) hexKey.append("0");
...
byte *
BLOWFISH::Encrypt_ECB (byte * data, int length, int *newlength)
{
  return Crypt_ECB (data, length, newlength, &BLOWFISH::Encrypt_Block, false);
}

byte *
BLOWFISH::Decrypt_ECB (byte * data, int length, int *newlength)
{
  return Crypt_ECB (data, length, newlength, &BLOWFISH::Decrypt_Block, true);
}

byte *
BLOWFISH::Encrypt_CBC (byte * data, int length, int *newlength)
{
  return Crypt_CBC (data, length, newlength, &BLOWFISH::Encrypt_Block, false);
}

byte *
BLOWFISH::Decrypt_CBC (byte * data, int length, int *newlength)
{
...
std::string BLOWFISH::Encrypt_CBC (std::string data)
{
  ...
  int
    newlen = 0;
  byte *
    result = Encrypt_CBC (binaryData, data.length (), &newlen);
  std::string encoded = "";
...
std::string BLOWFISH::Decrypt_CBC (std::string data)
{
  ...
  int
    len = 0;
  byte *
    cryptresult = Decrypt_CBC (binaryData, data.length () / 2, &len);
  std::string result = "";
  for (int i = 0; i < len; i++)
...
byte *
BLOWFISH::padData (byte * data, int length, int *paddedLength, bool decrypt,
		   bool IvSpace = false)
{
  int offset = 0;
  int dataoffset = 0;
...
int
BLOWFISH::findPaddingEnd (byte * data, int length)
{
  int i = length;
...
byte *
BLOWFISH::Crypt_ECB (byte * data, int length, int *newlength,
		     void (BLOWFISH::*CryptBlock) (byte *, int), bool decrypt)
{
  byte *pData;
...
byte *
BLOWFISH::Crypt_CBC (byte * data, int length, int *newlength,
		     void (BLOWFISH::*CryptBlock) (byte *, int), bool decrypt)
{
  byte *pData;
  if (!decrypt && !IvSet)
...
void
BLOWFISH::Encrypt_Block (byte * block, int offset)
{
...
void
BLOWFISH::Decrypt_Block (byte * block, int offset)
{
  setblock (block, offset);
  decipher ();
  getblock (block, offset);
...
void
BLOWFISH::setblock (byte * block, int offset)
{
...
void
BLOWFISH::getblock (byte * block, int offset)
{
  //TODO: CHECK ENDIANNESS
...
void
BLOWFISH::SetupKey (byte * cipherKey, int length)
{
  if (length > 56)
    {
      throw 56;
  ...
  for (int i = 0; i < 18; i++)
    {
      d =
	(((key[j % length] * 256 + key[(j + 1) % length]) * 256 +
	  key[(j + 2) % length]) * 256 + key[(j + 3) % length]);
      p[i] ^= d;
      j = (j + 4) % length;
    }
...
unsigned int
BLOWFISH::round (unsigned int a, unsigned int b, unsigned int n)
{
  //TODO: CHECK ENDIANNESS
  unsigned int x1 =
